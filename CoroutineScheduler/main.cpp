...
#elif defined(__linux__) && defined(__IOURING__) // clang++ main.cpp -luring -fcoroutines -std=c++23 -D__IOURING__
/* SPDX-License-Identifier: MIT */
/*
 * Very basic proof-of-concept for doing a copy with linked SQEs. Needs a
 * bit of error handling and short read love.
 * IO-Uring Code Ref: https://github.com/axboe/liburing/blob/master/examples/link-cp.c
 */
#include <stdio.h>
#include <fcntl.h>
#include <string.h>
...
class Scheduler {
    ...
    bool stop = false;
    // 之所以用taskCount来判断任务，因为AIO、IOURING同时复制大量文件会导致队列满堵塞
    // 在完成之前的任务之前，用tasks的size是不准确的（调度的时候会取出），只有协程销毁
    // 的时候才是协程任务真正减少了
    std::atomic<int> taskCount{0};

    void reschedule(Task::handle_type task) {
...
// Queue full when we have large file list
#if defined(__IOURING__) || defined(__AIO__)
        while (taskCount > 100) {
    ...
    void run() {
        while (true) {
            Task::handle_type task;
            {
                ...
                cv.wait(lock, [this]() { return taskCount > 0 || stop; });
                if (stop && taskCount == 0) return;
                task = tasks.front();
                tasks.pop();
            }
...
#if defined(__APPLE__) && defined(__GCD__)
...
#elif defined(__linux__) && defined(__IOURING__)
#define QD    64
#define BS    (4096)
...
static int handle_cqe(struct io_uring *ring, struct io_uring_cqe *cqe,
    ...
    if (cqe->res < 0) {
        if (cqe->res == -ECANCELED) {
            // TODO: 使用全局IOUring Queue，不再需要infd和outfd，事件处理可以交给单独的线程处理
            queue_rw_pair(ring, BS, data->offset, infd, outfd);
            inflight += 2;
        } else {
            printf("cqe error: %s\n", strerror(cqe->res));
            ret = 1;
...
Task copy_file1(const fs::path src, const fs::path dst) {
    ...
    while (insize) {                      // 数据还没处理完
        ...
        while (insize && inflight < QD) { // 数据还没处理完，io_uring 队列也还没用完
            ...
            queue_rw_pair(&ring, this_size, offset, in_fd, out_fd); // 创建一个 read->write chain，占用两个 SQE
            offset += this_size;
            insize -= this_size;
